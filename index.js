...
const LifxLight = require('node-lifx').Light;
const LifxPacket = require('node-lifx').packet;
const LifxConstants = require('node-lifx').constants;
...
class LifxAccessory {
    constructor(log, accessory, bulb, data) {
    ...
    setWaveform(hue, period, cycles, skewRatio, waveform, callback) {
        let light = this.accessory.getService(Service.Lightbulb);

        let packetObj = LifxPacket.create('setWaveform', {
            isTransient: true,
            color: {
    ...
    updateReachability(bulb, reachable) {
